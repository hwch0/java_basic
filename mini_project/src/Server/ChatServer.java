...
import Member.Room;
//import project.server.ChatRoom;
//import project.server.SocketClient;
...
public class ChatServer {
	...
	Map<String, Room> chatRooms = Collections.synchronizedMap(new HashMap<>()); // 채팅방 목록
	static int count=1;
	
	// DB (회원목록, 채팅목록)
	final MemberDB memberDB = new MemberDB();
	final String fileName = "C:/Users/KOSA/Temp/memberNew.db"; // 회원 목록
...
//	public Map<String, Member> readAllmembers() {
	...
	public void readAllrooms() {
	      ...
	      if (file.exists()){
	  		try {
				ois = new ObjectInputStream(new BufferedInputStream(new FileInputStream(file)));
				chatRooms = (Map<String, Room>) ois.readObject();
				ois.close();
			} catch (IOException|ClassNotFoundException e) {
...
//					e.printStackTrace();
	...
	//메소드: 채팅방 생성 
	public Room addChatRoom(String roomName) {
		Room chatRoom = new Room(roomName);
		if (chatRoom != null) {
			chatRooms.put(chatRoom.getRoomId(), chatRoom);
			System.out.println("채팅방 생성: " + roomName);
			System.out.println("현재 채팅방  수: " + chatRooms.size() + "\n");
		}
		return chatRoom;
	}
...
//	}
	...
	public void sendToAll(SocketClient sender, String message) {
		...
		Room chatRoom = chatRooms.get(sender.getChatRoomId());
		System.out.println("서버 sendToAll" + sender.toString() + message);
		if (chatRoom != null) {
	...
	public void stop() {
		try {
			...
			chatRooms.values().stream().forEach(sc -> sc.stop()); // 채팅방에 연결된 소켓 스트림으로 하나씩 가져와서 종료
